...
{
    public partial class TrackerViewerForm : Form, IBugRequester
    {
        ...
        public TrackerViewerForm(Project model)
        {
            InitializeComponent();
            ...
            if(GlobalConfig.EmailEnabled())
            {
                emailTime = GlobalConfig.EmailTime;

                emailTimer = new Timer();
                emailTimer.Interval = 60000; // 1 min
                emailTimer.Tick += new EventHandler(timer2_Tick);
                emailTimer.Start();
            }
        ...
        private void ManageEmailing()
        {
            if(emailTime > TimeSpan.MinValue)
            {
                foreach (Bug task in bugList)
                {
                    DateTime date1 = new DateTime();
                    DateTime date2 = new DateTime();
                    date1 = date1.AddDays(task.TimeToFinish.Days).AddHours(task.TimeToFinish.Hours).AddMinutes(task.TimeToFinish.Minutes);
                    date2 = date2.AddDays(emailTime.Days).AddHours(emailTime.Hours).AddMinutes(emailTime.Minutes);

                    if(date1 == date2)
                    {
                        TaskLogic.SendEmail(task, currentProject);
                    }
                }
            }
            
        }
        private void PopulateRecentlyEndedTasks()
        {
            List<Bug> recentlyEndedTasks = GlobalConfig.Connection.GetRecentlyFinishedTasks(currentProject);

            foreach(Bug task in recentlyEndedTasks)
            {
                if(task.TimeToFinish.Days > -1)
                {
                    if(task.FinishTime > DateTime.MinValue)
                    {
                        TimeSpan timeAfterEnd = task.FinishTime.Subtract(DateTime.Now);
                        if(timeAfterEnd.Days > -1)
                            finishedBugs.Add(task);
                    }
                    else
                        failedBugs.Add(task);
                }
        ...
        private void AddToFailedBugList()
        {
            foreach (Bug bug in bugList.ToList())
            {
                if (bug.TimeToFinish <= TimeSpan.Zero)
                {
                    ...
                    GlobalConfig.Connection.EndTask(bug);
            ...
            if (BugListView.SelectedItems.Count > 0)
            {
                ...
                if (box == DialogResult.Yes)
                {
                    foreach (ListViewItem item in BugListView.SelectedItems)
                    {
                        ...
                        GlobalConfig.Connection.DeleteTask(toRemove);
                    }
                    PopulateFirstList();
        ...
        private void FinishBugs()
        {
            if (BugListView.SelectedItems.Count > 0)
            {
                foreach (ListViewItem item in BugListView.SelectedItems)
                {
                    ...
                    SetFinishedListColor();
                    GlobalConfig.Connection.FinishTask(toRemove);
                }
                PopulateSecondList();
                PopulateFirstList();
        ...
        private void TrackerViewerForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            if(emailTimer != null)
                emailTimer.Stop();
            if(currentProject.Tasks != null)
                GlobalConfig.Connection.UpdatePriorities(currentProject);
        }
